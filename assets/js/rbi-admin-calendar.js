(function(){
  ...
  const DEFAULTS = COURSES.map((curso,i)=>({id:curso.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'').replace(/[^a-z0-9]+/g,'-').replace(/^-|-$/g,''),curso,fecha:'',hora:'',ciudad:'Chihuahua',modalidad:'Presencial',estado:'Por confirmar',color:COLORS[i]||'gold',linkWhatsapp:`https://wa.me/526141964438?text=${encodeURIComponent('Hola, quiero información sobre '+curso+' RBI.')}`,linkPago:'',visible:true,notas:''}));
