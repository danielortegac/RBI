(function(){
  ...
  const DEFAULTS = COURSES.map((curso,i)=>({id:curso.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'').replace(/[^a-z0-9]+/g,'-').replace(/^-|-$/g,''),curso,fecha:'',hora:'',ciudad:'Chihuahua',modalidad:'Presencial',estado:'Por confirmar',color:COLORS[i]||'gold',linkWhatsapp:`https://wa.me/525555020802?text=${encodeURIComponent('Hola, quiero información sobre '+curso+' RBI.')}`,linkPago:'',visible:true,notas:''}));
  ...
  function statusOptions(v){ return ['Activo','Próximo','Cupos limitados','Agotado','Por confirmar','Finalizado'].map(x=>`<option ${x===v?'selected':''}>${x}</option>`).join(''); }
  function modalOptions(v){ return ['Presencial','Online','Híbrido','Privado B2B','Por confirmar'].map(x=>`<option ${x===v?'selected':''}>${x}</option>`).join(''); }
  ...
  $('#addRowBtn')?.addEventListener('click',()=>{events.push({...DEFAULTS[0],id:'evento-'+Date.now(),fecha:'',hora:'',estado:'Por confirmar',visible:true}); render();});
